...
public class PersonalActivity extends BaseActivity<MinePresenter> implements MineContract.View {
    ...
    protected void initData() {
        if (userid!=0){
            showDialog();
            mPresenter.getUserDetail(userid,cookie_string);
        }
    }
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.ll_user_detail_follow:
                followUser(userid,1,cookie_string);
                break;
    ...
    private void followUser(long id,int t,String cookie){
        ApiService service = ApiEngine.getInstance().getApiService();
        service.followUser(id,t,cookie)
